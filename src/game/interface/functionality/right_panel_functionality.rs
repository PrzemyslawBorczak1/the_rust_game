...
use crate::{
    data::{GPUMaterial, GameState, GameWorld},
    game::interface::common::InterfaceState,
};
use bevy::prelude::*;
...
impl Plugin for RightPanelFunctionalityPlugin {
    fn build(&self, app: &mut App) {
        ...
        .add_systems(
            Update,
            (change_ui_visibility).run_if(in_state(GameState::Game)),
        );
    }
}
...
pub fn on_political_click(
    ...
    for interaction in &mut q {
        if *interaction == Interaction::Pressed {
            if let Some(gpu) = materials.get_mut(world.gpu.id()) {
